...
def day_of_week_date_ui(preset_options, default_preset_index, min_days):
    """
    ...
    preset = st.sidebar.selectbox("Weekly Date Ranges", all_options, default_preset_index)
    ...
    custom_range = st.sidebar.checkbox("Select specific week", value=(preset == "Custom Range"))
    
    if custom_range:
        # Show predefined weekly options for easier selection
        weeks_ago = st.sidebar.slider("Weeks before cutoff date", 0, 52, 1)
        ...
        manual_override = st.sidebar.checkbox("Manual date override")
        if manual_override:
            start = st.sidebar.date_input("Start Date", start_of_week, format="MM/DD/YYYY", max_value=config.cutoff_date)
